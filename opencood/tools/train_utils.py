...
def load_whole_model(saved_path, model):
    epoch = saved_path.split('epoch')[-1].split('.')[0]
    print('resuming by loading epoch %s' % epoch)
    device = torch.device("cpu")
    state_dict_ = torch.load(saved_path, map_location=device)
    ...
    # convert data_parallal to model
    for k in state_dict_:
        if 'model_infra' not in k and 'model_vehicle' not in k and 'cls_head' not in k and 'reg_head' not in k and 'fusion_net' not in k:   # 加载完整的
            continue
        if k.startswith('module') and not k.startswith('module_list'):
            state_dict[k[7:]] = state_dict_[k]
        else:
